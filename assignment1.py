...
def beta_function(x, w, beta, is_using):
  ## Constant 
  return beta
...
def run_experiment(alphas, betas):
  ...
  for iteration in range(MAXITERATION):
    if DYNAMIC_MODE == 1:
      ...
      for idx in np.where( (roulette <= probability) == True):
        probability[idx] = 0
    ...
    if x.sum() > CAPACITY:
      betas2 = np.array([beta_function(x, w, b, is_using) for x, w, b in zip(x, w_k, betas)])
      x *= betas2
      w_k = x.copy()    
      is_md[iteration] = 1
      # print("MD")
